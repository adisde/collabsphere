...
export const adminAuthorize = async (req, res, next) => {
  try {
    ...
    if (isExistMemberForProject.role !== "admin") return res.status(401).json({ ok: false, message: "Access denied" });

    req.id = project_id;
    req.user_id = user_id;
    next();
  } catch (err) {
    console.error("Project admin validation error:", err.message);
